...
public class TetrisBoard implements Serializable{
    ...
    public boolean getGrid(int x, int y) {
        if (x >= this.width || x < 0 || y >= this.height || y < 0 || tetrisGrid[x][y])
            return true;
        return false;
    }
    ...
    public int placePiece(TetrisPiece piece, int x, int y) {
        ...
        for (TetrisPoint p : piece.getBody()) {
            ...
            // x,y + current location x,y  >  bound x,y
            if ((x + p.x) > this.width || (x + p.x) < 0 || (y + p.y) > this.height ||
                    (y + p.y) < 0){
                return ADD_OUT_BOUNDS;
            }

            // if grid[piece x,y + location x,y] = True, then bad
            if (this.getGrid(x + p.x, y + p.y)) {
                return ADD_BAD;
            }
        }
        ...
        // successfully added it, nothing amazing happen
        return ADD_OK;
    }
    ...
    public int clearRows() {
        ...
        // set false for the fulfilled rows
        int numRowCleared = 0;
        for (int ii = 0; ii < this.tetrisGrid[0].length; ii++) {
            if (this.rowCounts[ii] == this.width) {
                for (int i = 0; i < this.tetrisGrid.length; i++) {
                    this.tetrisGrid[i][ii] = false;
                }
                this.rowCounts[ii] = 0;
                numRowCleared += 1;
            }
        ...
        for (int i = 0; i < this.tetrisGrid.length; i++) {
            placeTrueHere = 0;
            for (int ii = 0; ii < this.tetrisGrid[0].length; ii++) {
                if (tetrisGrid[i][ii] == true && placeTrueHere == ii) { // now is true, no need to shift to left
                    placeTrueHere++;
                } else if (tetrisGrid[i][ii] == true && placeTrueHere != ii) { // now is true, it needs to shift to left
                    tetrisGrid[i][placeTrueHere] = true;
                    tetrisGrid[i][ii] = false;
                    placeTrueHere++;
                }
            }
        }
        return numRowCleared;
    }
